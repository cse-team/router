...
public class CHMatrixGeneration {
	 ...
	 */
	public static final String VEHICLE_TYPE_CAR = FlagEncoderFactory.CAR;
	public static final String VEHICLE_TYPE_BIKE = FlagEncoderFactory.BIKE;
	public static final String VEHICLE_TYPE_FOOT = FlagEncoderFactory.FOOT;
	public static final String VEHICLE_TYPE_BIKE2 = FlagEncoderFactory.BIKE2;
	public static final String VEHICLE_TYPE_MOTORCYCLE = FlagEncoderFactory.MOTORCYCLE;
	public static final String VEHICLE_TYPE_RACINGBIKE = FlagEncoderFactory.RACINGBIKE;
	public static final String VEHICLE_TYPE_MOUNTAINBIKE = FlagEncoderFactory.MOUNTAINBIKE;
	...
	public static EncodingManager createEncodingManager(String directory) {
        ...
        for(String encoderString: allEncoderString.split(",")){
        	...
        	final String variant = splitTypeVariant.length>1? splitTypeVariant[1].trim():"";
        	...
        	final ToStringHelper h= new ToStringHelper();
        	
        	FlagEncoder encoder = null;
            if (type.equals(FlagEncoderFactory.CAR)){
            ...
            else if (type.equals(FlagEncoderFactory.BIKE)){
            ...
            else if (type.equals(FlagEncoderFactory.FOOT)){
            ...
            else if (type.equals(FlagEncoderFactory.MOTORCYCLE)){
	...
	public ShortestPathTree search(int startNode, EdgeExplorer edgeExplorer, EdgeFilter edgeFilter, boolean isBackwards) {
		...
		PriorityQueue<SPTEntry> openSet = new PriorityQueue<SPTEntry>();
	...
	public MatrixResult calculateMatrix(GHPoint[] points, CHProcessingApi processingApi) {
		...
		final TIntObjectHashMap<List<FromIndexEdge>> visitedByNodeId = new TIntObjectHashMap<List<FromIndexEdge>>();
		if (processingApi != null) {
	...
	private MatrixResult searchAllBackward(QueryResult[] snapToResults, final QueryGraph snapToGraph, final ShortestPathTree[] forwardTrees,
		...
		if (useExpansionCache) {
			expansionCache = new HashMap<EdgeExpansionCacheKey, DistanceTime>();
		} else {
			expansionCache = null;
		}
			...
			if (snapToResults[fromIndex].isValid()) {
				...
				forwardTrees[fromIndex].forEachEntry(new TIntObjectProcedure<SPTEntry>() {
					...
					public boolean execute(int nodeId, SPTEntry edge) {
						...
						if (visited == null) {
							visited = new ArrayList<FromIndexEdge>(1);
